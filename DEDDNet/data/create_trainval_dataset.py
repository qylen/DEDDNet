...
import numpy as np
import random
from DPTmodel.dpt.transforms import Resize, NormalizeImage, PrepareForNet
...
class TrainDataset(Dataset):
    def __init__(self, trainopt):
        ...
        self.DPTnormalization = NormalizeImage(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        self.DPTtransform = Compose(
                [
                    Resize(
                        384,
                        384,
    ...
    def __getitem__(self, index): #没有用数据增强
        ...
        DPT_img = cv2.cvtColor(cs_img, cv2.COLOR_BGR2RGB) / 65535.  
        DPT_img_input = self.DPTtransform({"image": DPT_img})["image"]
        sample = torch.from_numpy(DPT_img_input)
        ...
        return l_img, r_img, b_img, gt_img,cs_img,sample
...
class ValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return l_img, r_img, b_img, gt_img,cs_img, os.path.basename(c_img_name).split('.'),sample
