...
#构造巴特沃斯高通滤波器，首先用估计的模糊核代替高斯模糊核，然后无监督估计模糊核，必要时可以使用视差估计，估计COC，用面片状的方式进行监督
class GemorestructNet(nn.Module):
    def __init__(self):
        ...
        for i in range(0, self.radius):
            ...
            if k==0:
                ...
                current_conv = nn.Conv2d(1, 1, kernel_size=k * 2 + 1, bias=False)  #可以把这个模糊核用巴特沃斯核替代
    ...
    def radius_dictA(self, c_radius):
        ...
        for i in range(1,c_radius):
            ...
            radius_set.append(torch.tensor(kernel_c).float()) #这应该是个高斯模糊核
        return radius_set
    ...
    def radius_dictA2(self, radius_set,c_radius):
        ...
        return radius_set, weight_pos_set, weight_neg_set #输出了一个高斯核，一个左图的模糊核，一个右图的模糊核,先用一个巴特沃斯核看一看
        ...
        for i in range(self.radius):
            if i ==0:
               k=0
                ...
                k=k+1
            current_left = F.pad(x[:, :3, :, :], pad=(k, k, k, k)) #padding和smooth有关了
