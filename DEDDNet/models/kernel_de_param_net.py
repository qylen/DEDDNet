...
class KernelEDNet(nn.Module):
    def __init__(self):
        ...
        for k in kernel_size:
            self.layer1.append(kernel_conv(k, self.channel, 16, max_pool=True))

        self.layer2 = nn.ModuleList()
        for k in kernel_size:
            self.layer2.append(kernel_conv(k, self.channel, 16, max_pool=True))

        self.layer3 = nn.ModuleList()
        for k in kernel_size:
            self.layer3.append(kernel_conv(k,self.channel, 16, max_pool=True))

        self.layer4 = nn.ModuleList()
        for k in kernel_size:
            self.layer4.append(kernel_conv(k, self.channel, 16, max_pool=True))

        self.layer5 = nn.ModuleList()
        for k in kernel_size:
            self.layer5.append(kernel_conv(k, self.channel, 16, upsample=True))

        self.layer6 = nn.ModuleList()
        for k in kernel_size:
            self.layer6.append(kernel_conv(k, self.channel, 16, upsample=True))

        self.layer7 = nn.ModuleList()
        for k in kernel_size:
            self.layer7.append(kernel_conv(k, self.channel, 16, upsample=True))

        self.layer8 = nn.ModuleList()
        for k in kernel_size:
            self.layer8.append(kernel_conv(k, self.channel, 16, upsample=True))

        self.MAX_TRAINNUM = 2.5e4
        self.iter_num = 0

    def forward(self, x,s1=1.8,s2=4.5,s3=6.9, gt=None):
        blur, _ = x[:, 3:, :, :].abs().max(dim=1, keepdim=True)
        x = x[:, :3, :, :]
        x = self.head(x)
        blur_mask = []

        static_kernel_size =[0.0,s1,s2,s3] #[0.0,1.9,4.2,6.2]
